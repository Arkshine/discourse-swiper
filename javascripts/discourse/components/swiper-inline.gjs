...
export default class SwiperInline extends Component {
  ...
  async initializeSwiper(element) {
    ...
    console.log("thumb slider", this.thumbSlider);
    ...
    function hoverThumbs({ swiper, extendParams, on }) {
      ...
      on("init", function () {
        ...
        const thumbsSwiper = params.swiper;

        // Add hover listeners to each thumbnail slide
      ...
      on("destroy", function () {
        ...
        // Remove hover listeners from each thumbnail slide
        thumbsSwiper.slides.forEach((slide, index) => {
    ...
    this.mainSlider = new window.Swiper(slideElement, {
      ...
      hoverThumbs: {
        enabled: this.config.showThumbs,
