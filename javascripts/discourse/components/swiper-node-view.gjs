...
export default class SwiperNodeView extends Component {
  ...
  toggleEditMode() {
    ...
    if (this.isEditMode) {
      ...
      view.dispatch(this.markNodeMode("edit").scrollIntoView());
      view.dispatch(
      ...
      view.dispatch(this.markNodeMode("view").scrollIntoView());
      view.focus();
    }
  }
  ...
  async openSettings() {
    ...
    const closeSettingsMenuBind = this.closeSettingsMenu.bind(this);

    withPluginApi((api) => {
      console.log("rendering in outlet");

      api.renderInOutlet(
        "composer-after-composer-editor",
        <template>prout</template>
      );
    });
