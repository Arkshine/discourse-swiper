...
import { SETTINGS_EFFECT_DISABLES as EFFECT_DISABLES } from "../lib/constants";
...
export default class SwiperInlineSettings extends Component {
  ...
  shouldDisable(name) {
    ...
    if (EFFECT_DISABLES[effect]?.has(name)) {
      return true;
    }

    if (name === "slidesPerGroup") {
      return parseInt(this.config.slidesPerView, 10) === 1;
    }
  ...
  get disableMap() {
    return Object.fromEntries(
      Object.keys(EFFECT_DISABLES).flatMap((effect) =>
        [...EFFECT_DISABLES[effect]].map((name) => [
          name,
  ...
  updateSetting(key, value) {
    ...
    if (key === "direction" && value === "vertical") {
      this.applySetting("slidesRows", "1");
    }
  }
  ...
  <template>
    ...
    <div class="settings-section-content">
      ...
      {{#if (eq this.activeCategory "thumbnail")}}
        <div class="setting-row">
          <div class="setting-label">
            {{icon "swiper-enable-thumbnail" class="setting-icon"}}
        ...
        <div class="setting-row">
          <div class="setting-label">
            {{icon "swiper-thumbnail-slides-per-view" class="setting-icon"}}
        ...
        <div class="setting-row has-range-input">
          <div class="setting-label">
            {{icon "swiper-thumbnail-space-between" class="setting-icon"}}
        ...
        <div class="setting-row">
          <div class="setting-label">
            {{icon "swiper-thumbnail-direction" class="setting-icon"}}
